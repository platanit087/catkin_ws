...
void LowLevelControl::SetRobotParams(float diameter) {
	...
	std::cout << classPrompt << "Setting parameters: RobotDiam: "
			<< this->robotDiam << std::endl;
}

void LowLevelControl::CalculateSpeeds(float robotX, float robotY,
		float robotTheta, float goalX, float goalY, float& lSpeed,
		float& rSpeed, bool backwards) {
	...
	if (this->controlType == CTRL_EXPONENTIAL) {
		...
		float exp_MaxLinear =
				distError < this->MaxLinear ? distError : this->MaxLinear;
		if (exp_MaxLinear < 0.25f)
			exp_MaxLinear = 0.25f;
		...
		float expTrans = -(angError * angError)
				/ (2 * this->exp_alpha * this->exp_alpha);
...
void LowLevelControl::CalculateSpeeds(float currentTheta, float goalAngle,
...
void LowLevelControl::CalculateSpeeds(float robotX, float robotY,
		float robotTheta, float goalX, float goalY, float& lSpeed,
		float& rSpeed) {
	this->CalculateSpeeds(robotX, robotY, robotTheta, goalX, goalY, lSpeed,
			rSpeed, false);
}
